...
class URIHelperSlp() {
    ...
    constructor(uri: String, tryForSend: Boolean) : this() {
        ...
        if (!uri.contains("http")) {
            if (mappedVariables["amount"] != null) {
                SendSlpTokenController.INSTANCE.sendType!!.value =
                ...
                SendSlpTokenController.INSTANCE.toAmount!!.text = amount
            } else {
                amount = "null"
            }
    ...
    fun processSendAmount(amount: String): String {
        return when (SendSlpTokenController.INSTANCE.sendType!!.value) {
