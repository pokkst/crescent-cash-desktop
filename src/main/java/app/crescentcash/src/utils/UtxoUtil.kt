...
class UtxoUtil {
    ...
    fun setupSweepWallet(key: ECKey) {
        walletToSweep = Wallet.createBasic(WalletHelper.parameters)
    ...
    fun sweep(toAddress: String) {
        ...
        println(txHex)
        Main.INSTANCE.walletHelper.broadcastTxToPeers(tx)

        if (!WalletHelper.useTor) {
            Main.INSTANCE.netHelper.broadcastTransaction(txHex, "https://rest.bitcoin.com/v2/rawtransactions/sendRawTransaction")
        }

        Main.INSTANCE.netHelper.broadcastTransaction(txHex, "https://rest.imaginary.cash/v2/rawtransactions/sendRawTransaction")
    }
    ...
    private fun getTransaction(txId: String): Transaction? {
        ...
        try {
            `is` = if (WalletHelper.useTor) URL(url).openConnection(WalletHelper.torProxy).getInputStream() else URL(url).openConnection().getInputStream()
        } catch (e: IOException) {
            e.printStackTrace()
        }
